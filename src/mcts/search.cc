...
void WriteGMLNode(int id, std::string move_string, std::string N_string, std::string Q_string, std::string D_string, std::string M_string, std::string P_string, std::ofstream& file){
 std::string quot = "\"";
 file << "node" << std::endl;
 file << "[" << std::endl;
 ...
 file << "M " + quot + M_string + quot << std::endl;
...
void RecursiveGMLWrite(Node* node, std::ofstream& file, bool flip, int *id, int parent_id){
    ...
    M_string = std::to_string(node->GetM());
    WriteGMLNode(*id, move_string, N_string, Q_string, D_string, M_string, P_string, file);
    parent_id = *id;
    *id = *id + 1;
    flip = !flip;
    for (const auto& child : node->Edges()) {
